...
def checkout():
    ...
    if form.validate_on_submit():
        return render_template("checkout.html",name=name,form = form, action ={{redirect(url_for('confirm'))}})
      
@app.route("/blah")
def confirm():
    return render_template('confirm.html'),'Dear {}, your order for {} drinks is placed Succesfully.'.format(form.name.data,form.total.data) 
